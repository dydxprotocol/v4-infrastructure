...
resource "aws_db_parameter_group" "upgrade" {
  name_prefix = "${var.environment}-${var.indexers[var.region].name}-db-parameter-group-upgrade-16"
  ...
  tags = {
    Name        = "${var.environment}-${var.indexers[var.region].name}-db-parameter-group-upgrade-16"
    Environment = var.environment
  }
}
